...
wa_model <- lm(data = wa_Alaska_Native, PERNP ~ .)
...
library(lmtest)
bptest(wa_model)
bptestequation = lm( data=wa_Alaska_Native, residuals(wa_model)*residuals(wa_model) ~. - PERNP)
...
#### Model 1-2
```{r}
wa_model_rvs <- lm(data = wa_Alaska_Native, PERNP ~ . + I(AGEP^2))
...
```{r}
earn_edu_pre <- data.frame("EarnPre"= predict(wa_model_rvs), "Residual" = residuals(wa_model_rvs), "stan.Res" = rstandard(wa_model_rvs), "Fitted"=wa_Alaska_Native$PERNP )
...
#Conduct BPG Test
bptest(wa_model_rvs)
bptestequation1.2 = lm( data = wa_Alaska_Native, residuals(wa_model_rvs)*residuals(wa_model_rvs) ~. + I(AGEP^2) -PERNP)
...
```{r}
logEarning.model <- lm( data = wa_Alaska_Native, log(PERNP)~ . +I(AGEP^2))
...
bptestequation1.3 = lm( data = wa_Alaska_Native, residuals(logEarning.model)*residuals(logEarning.model) ~. + I(AGEP^2) -PERNP)
summary(bptestequation1.3) #note: BP = n*RSquared of model with squared residuals as dependent variable 
```
studentized Breusch-Pagan test

data:  logEarning.model
BP = 17.647, df = 13, p-value = 0.1714
